...
const { packs, currentPage, lastPage, packLoading: loader } = packApi;
...
watch(input, async () => {
  await packApi.filterPack(input.value);
});
      ...
      <div class="menu">
        ...
        <div class="pagination">
          <Pagination
            ...
            :lastPage="lastPage"
            @next="packApi.nextPage"
            @prev="packApi.prevPage"
            @goTo="packApi.gotoPage"
          />
        </div>

        <div v-if="!loader" class="data-list" i>
          <ul>
            <li v-for="pack in packs" @click="onSelect(pack)" :key="pack.id">
              {{ pack.title }} ({{ pack.quantity }})
            </li>
          </ul>
        </div>
